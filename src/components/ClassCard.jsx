...
const handleSelectClass = () =>{
  if (!user) {
    ...
    return (
        <div className={`shadow-2xl rounded-xl  p-6 space-y-2 flex flex-col h-[400px]`}>
      ...
      <div className={`mt--auto flex flex-col items-start`}>
        ...
        {
            isAdmin &&
            <p className="mb-2">Instructor Email: 
          {instructorEmail}
        </p>
        ...
        <p className="mb-2">Available seats :  
        {availableSeats?` ${ availableSeats}`:' None'}
        </p>
        <p className="mb-2">Price : $
          {price}
        </p>
        {isInstructor && <p className="mb-2">Enrolled students: 
          { enrolledStudents ? enrolledStudents : '0'}
        </p>}
        
        {(isAdmin || isInstructor) && <p className="mb-2">Status : 
          {status}
        </p>}
        {
            isAdmin && <>
            <div className="flex flex-col md:flex md:flex-row gap-2 items-center justify-center mb-2">
      ...
      {isOpen && (
       ...
       <button onClick={handleSelectClass} disabled={isAdmin || isInstructor ||( availableSeats == 0)} className={`mt-2 py-2 px-4 bg-red-500 hover:bg-red-600 text-white rounded-lg mx-auto ${location?.pathname == '/dashboard/manageclasses'?'hidden':''}`}>Select</button>
